        ...
        while len(buffer) >= packet_size:
            ...
            if len(result) == 1: 
                if result[0] not in dic:
                    if result[0] == "MIDCAPS":
                        dic["MIDCAPS"] = prev_close_price
                        dic["MIDCAP"] = prev_close_price
                    else:
                        dic[result[0]] = prev_close_price
    ...
    while True:
        ...
        while len(buffer) >= packet_size:
            ...
            if cp == 'XX':
                ...
                except (BelowIntrinsicException, AboveMaximumException, ZeroDivisionError, ValueError):
                    iv = '-'

            try:
                change_interest = str(
                    round((open_interest-previous_open_interest)/previous_open_interest, 2)) + "%"
